const Nosotros = () =>{
    return(
        <div className="containter">
            <div className="row">
                <div class="col-lg-6 col-12 texto">
				    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Itaque velit fuga expedita placeat molestiae. Exercitationem corporis cumque, accusamus placeat dicta quibusdam magni perspiciatis totam autem rem quam. A cum, enim.
				    </p>
			    </div>
            </div>


            <div class="col-lg-6 col-12">
				<iframe class="video" width="560" height="315" src="https://www.youtube.com/embed/hY5abbglUJ0" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
			</div>
        </div>
    )
}

export default Nosotros;